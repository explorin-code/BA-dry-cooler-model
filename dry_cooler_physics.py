from cmath import log10, pi, ln, tanh
...
def calc_Nu_turbulent(Re: float, Pr: float, d_i: float, L: float):
    ...
    Nu_mT = (
        ((psi / 8) * (Re - 1000) * Pr)
        / (1 + 12.7 * (psi / 8) ** 0.5 * (Pr ** (2 / 3) - 1))
    ) * (1 + (d_i / L) ** (2 / 3))  # Gnielinski equation for turbulent flow with correction factor (G1 26)
...
def calc_Nu_laminar(Re: float, Pr: float, d_i: float, L: float):
    ...
    Nu_mq3 = 0.924 * Pr ** (1 / 3) * (Re * (d_i / L)) ** 0.5  # Nusselt number for developing laminar flow with constant heat flux and Prandtl number correction (G1 24)
    ...
    Nu_mL = (
        ...
        + (Nu_mq2 - 0.6) ** 3
        + Nu_mq3 ** 3
    ) ** (1 / 3)  # Combined Nusselt number for laminar flow with constant heat flux (G1 25)
...
def calc_alpha_i(
    ...
    rho, c_p, lambda_coolant, eta, Pr = get_coolant_properties(
        P_coolant, T_coolant, coolant
    )
    ...
    elif Re < 4000:  # Transitional flow (G1, 4.2)
        gamma = (Re - 2300) / (4000 - 2300)  # Transition parameter (G1 30)
        Nu = (
            (1 - gamma) * calc_Nu_laminar(2300, Pr, d_i, L)
            + gamma * calc_Nu_turbulent(4000, Pr, d_i, L)
        )  # Linear interpolation between laminar and turbulent Nusselt numbers (G1 29)
...
def get_geometry():  # Exemplary values for geometry from M1 4
    ...
    A_R = 2 * (pi / 4) * (D ** 2 - d ** 2) * fins_per_pipe  # Outer surface area of a segment of the tube between two ribs [m²]
